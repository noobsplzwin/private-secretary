...
const registry = readMatters(resolve(process.cwd(), "config/matters.yaml"));
const matters = [...activeMatterIds(registry)];
const closedMatters = [...closedMatterIds(registry)];
const input: EvalInput = { frozenAt: manifest.frozenAt, matters, closedMatters, persons };
